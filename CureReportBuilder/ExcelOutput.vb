...
Public Class ExcelOutput
    ...
    Sub formatFont(inCell As Object,
                   ...
                   Optional checkExists As Boolean = True)
    ...
    Sub outputResults(templatePath As String, checkToOutput As CureCheck, Optional outputPath As String = "")
        ...
        mainSheet.Cells(2, 1) = "Job" & vbNewLine & checkToOutput.JobNum & vbNewLine &
                                "Program" & vbNewLine & checkToOutput.ProgramNum

        formatFont(mainSheet.Cells(2, 1), "Job", 14, True, False, True)
        formatFont(mainSheet.Cells(2, 1), "Program", 14, True, False, True)

        mainSheet.Cells(2, 4) = "Part" & vbNewLine & checkToOutput.PartNum & vbNewLine &
                                "Rev. " & checkToOutput.PartRev & vbNewLine &
                                checkToOutput.PartNom & vbNewLine &
                                "Qty: " & checkToOutput.PartQty
        ...
        mainSheet.Cells(2, 8) = "Cure" & vbNewLine &
                                "Start: " & checkToOutput.dateArr(checkToOutput.cureStart).ToString("dd-MMM-yyyy | h:mm tt") & vbNewLine &
                                "Finish: " & checkToOutput.dateArr(checkToOutput.cureEnd).ToString("dd-MMM-yyyy | h:mm tt") & vbNewLine &
                                "Duration: " & hours & "h:" & minutes & "m"

        formatFont(mainSheet.Cells(2, 8), "Cure", 14, True, False, True)

        mainSheet.Cells(5, 1) = "Equipment" & vbNewLine &
                                checkToOutput.machType & " | " & checkToOutput.equipSerialNum
        ...
        mainSheet.Cells(5, 8) = "Cure Document" & vbNewLine & checkToOutput.curePro.cureDoc & vbNewLine &
                                "Rev. " & checkToOutput.curePro.cureDocRev & vbNewLine &
                                "Profile: " & checkToOutput.curePro.Name
        ...
        For i = 0 To UBound(checkToOutput.curePro.CureSteps)
            ...
            Call fillCell1(currentStep, mainSheet, checkToOutput, curRow)

            'Fill out cell 2
            Call fillCell2(currentStep, mainSheet, checkToOutput, curRow)

            'Fill out cell 3
            Call fillCell3(currentStep, mainSheet, checkToOutput, curRow)
        ...
        If My.Computer.FileSystem.FileExists(outputPath & "\CureReports\ExcelReports\CureReport_" & cureAcro & "_" & checkToOutput.JobNum & ".xlsx") _
            Or My.Computer.FileSystem.FileExists(outputPath & "\CureReports\CureReport_" & cureAcro & "_" & checkToOutput.JobNum & ".pdf") Then

            Dim idNum As Integer = 1
            Do While mismatchStr = ""
                If Not My.Computer.FileSystem.FileExists(outputPath & "\CureReports\ExcelReports\CureReport_" & cureAcro & "_" & checkToOutput.JobNum & "(" & idNum & ")" & ".xlsx") _
                            And Not My.Computer.FileSystem.FileExists(outputPath & "\CureReports\CureReport_" & cureAcro & "_" & checkToOutput.JobNum & "(" & idNum & ")" & ".pdf") Then
                    mismatchStr = "(" & idNum & ")"
                End If
                idNum += 1
            Loop
    ...
    Sub fillDataSheet(mainSheet As Excel.Worksheet, dataSheet As Excel.Worksheet, checkToOutput As CureCheck)
        dataSheet.Activate()
        ...
        outputDataToColumn(dataSheet,
                           curCol,
                           "Time " & vbNewLine & "Stamp" & vbNewLine & "(DateTime)",
                           arr2Str(checkToOutput.dateArr, checkToOutput),
                           checkToOutput.cureStart,
                           checkToOutput.cureEnd)

        outputDataToColumn(dataSheet,
                           curCol,
                           "Cure Time" & vbNewLine & "(min)",
                           arr2Str(checkToOutput.dateArr, checkToOutput,, True),
                           checkToOutput.cureStart,
                           checkToOutput.cureEnd)
        ...
        If checkToOutput.curePro.checkTemp Then
            If checkToOutput.machType = "Autoclave" Then
                outputDataToColumn(dataSheet,
                                   curCol,
                                   "Air TC" & vbNewLine & "(°F)",
                                   arr2Str(checkToOutput.vessel_TC.values, checkToOutput, 1),
                                   checkToOutput.cureStart,
                                   checkToOutput.cureEnd)
            ...
            If UBound(checkToOutput.usrRunTC) > 0 Then
                outputDataToColumn(dataSheet,
                                   curCol,
                                   "Lead TC" & vbNewLine & "(°F)",
                                   arr2Str(checkToOutput.leadTC.values, checkToOutput, 1),
                                   checkToOutput.cureStart,
                                   checkToOutput.cureEnd)
            ...
            For i = 0 To UBound(checkToOutput.partTC_Arr)
                ...
                For z = 0 To UBound(checkToOutput.usrRunTC)
                    If checkToOutput.partTC_Arr(i).Number = checkToOutput.usrRunTC(z) Then
                        outputDataToColumn(dataSheet,
                                           curCol,
                                           "TC " & checkToOutput.partTC_Arr(i).Number & vbNewLine & "(°F)",
                                           arr2Str(checkToOutput.partTC_Arr(i).values, checkToOutput, 1),
                                           checkToOutput.cureStart, checkToOutput.cureEnd)

                        outputDataToColumn(dataSheet,
                                           curCol,
                                           "TC " & checkToOutput.partTC_Arr(i).Number & " - Ramp" & vbNewLine & "(°F/min)",
                                           arr2Str(checkToOutput.partTC_Arr(i).ramp, checkToOutput, 2),
                                           checkToOutput.cureStart,
                                           checkToOutput.cureEnd)
                        ...
                        If UBound(checkToOutput.usrRunTC) = 0 Then
                            Call addToChart(checkToOutput,
                                            ...
                                            dataSheet,
                                            curCol - 2,
                                            "TC -" & checkToOutput.partTC_Arr(i).Number & "- (°F)",
                                            checkToOutput.partTC_Arr(i),
                                            1,
                                            False,
                                            Color.Blue)
                        End If
        ...
        If checkToOutput.curePro.checkPressure Then
            outputDataToColumn(dataSheet,
                               curCol,
                               "Pressure" & vbNewLine & "(psi)",
                               arr2Str(checkToOutput.vesselPress.values, checkToOutput, 1),
                               checkToOutput.cureStart,
                               checkToOutput.cureEnd)
            addToChart(checkToOutput,
                       mainSheet,
            ...
            outputDataToColumn(dataSheet,
                               curCol,
                               "Pressure Ramp" & vbNewLine & "(psi/min)",
                               arr2Str(checkToOutput.vesselPress.ramp, checkToOutput, 2),
                               checkToOutput.cureStart,
                               checkToOutput.cureEnd)
        End If
        ...
        If checkToOutput.curePro.checkVac Then
            If UBound(checkToOutput.usrRunVac) > 0 Then
                outputDataToColumn(dataSheet,
                                   curCol,
                                   "Max Vac" & vbNewLine & "(inHg)",
                                   arr2Str(checkToOutput.maxVac.values, checkToOutput, 1),
                                   checkToOutput.cureStart,
                                   checkToOutput.cureEnd)
                ...
                '                curCol - 1,
                '                "Max Vacuum (inHg)",
                '                checkToOutput.maxVac,
                '                2,
                '                False)

                outputDataToColumn(dataSheet,
                                   curCol,
                                   "Min Vac" & vbNewLine & "(inHg)",
                                   arr2Str(checkToOutput.minVac.values, checkToOutput, 1),
                                   checkToOutput.cureStart,
                                   checkToOutput.cureEnd)
            ...
            For i = 0 To UBound(checkToOutput.vac_Arr)
                ...
                For z = 0 To UBound(checkToOutput.usrRunVac)
                    If checkToOutput.vac_Arr(i).Number = checkToOutput.usrRunVac(z) Then
                        outputDataToColumn(dataSheet,
                                           curCol,
                                           "Vac Port " & checkToOutput.vac_Arr(i).Number & vbNewLine & "(inHg)",
                                           arr2Str(checkToOutput.vac_Arr(i).values, checkToOutput, 1),
                                           checkToOutput.cureStart,
                                           checkToOutput.cureEnd)
                        ...
                        If UBound(checkToOutput.usrRunVac) = 0 Then
                            Call addToChart(checkToOutput,
                                            ...
                                            dataSheet,
                                            curCol - 1,
                                            "Vac Port -" & checkToOutput.vac_Arr(i).Number & "- (inHg)",
                                            checkToOutput.vac_Arr(i),
                                            2,
                                            False,
                                            Color.Green)
                        End If
    ...
    Sub fillCell1(currentStep As CureStep, mainSheet As Excel.Worksheet, checkToOutput As CureCheck, ByRef curRow As Integer)
        ...
        Else
            ...
            mainSheet.Cells(curRow, 1) = currentStep.stepName & vbNewLine &
                                         "(" & stepPass & ")" & vbNewLine &
                                         "Start: " & startTime.ToString("0.0") & " min" & vbNewLine &
                                         endStr & durationStr
    ...
    Sub fillCell2(currentStep As CureStep, mainSheet As Excel.Worksheet, checkToOutput As CureCheck, ByRef curRow As Integer)
        Dim cell2 As String = ""
        ...
        If checkToOutput.curePro.checkTemp And currentStep.tempSet.SetPoint <> -1 Then
            ...
            Else
                ...
                tempStr = "Temperature " & currentStep.tempSet.SetPoint & "°F " &
                          plusMinusVal(currentStep.tempSet.PosTol, currentStep.tempSet.NegTol) & "°F"
            End If
            tempStr = tempStr & vbNewLine

            If currentStep.tempSet.RampRate <> 0 Then
                tempRmpStr = "Temp. Ramp " & currentStep.tempSet.RampRate & "°F/min " &
                             plusMinusVal(currentStep.tempSet.RampPosTol, currentStep.tempSet.RampNegTol) & "°F/min"
        ...
        If checkToOutput.curePro.checkPressure And currentStep.pressureSet.SetPoint <> -1 Then
            ...
            Else
                pressStr = pressStr & "Pressure:  " & currentStep.pressureSet.SetPoint & " psi " &
                           plusMinusVal(currentStep.pressureSet.PosTol, currentStep.pressureSet.NegTol) & " psi"
            End If
            ...
            If currentStep.pressureSet.RampRate <> 0 Then
                pressRmpStr = pressRmpStr & "Pressure Ramp " & currentStep.pressureSet.RampRate & " psi/min " &
                              plusMinusVal(currentStep.pressureSet.RampPosTol, currentStep.pressureSet.RampNegTol) & " psi/min"
        ...
        If checkToOutput.curePro.checkVac And currentStep.vacSet.SetPoint <> -1 Then
            ...
            Else
                vacStr = vacStr & "Vacuum:  " & currentStep.vacSet.SetPoint & " inHg " &
                         plusMinusVal(currentStep.vacSet.PosTol, currentStep.vacSet.NegTol) & " inHg"
            End If
        ...
        Dim term1Str As String = termToStr(currentStep.termCond1Type,
        ...
        If currentStep.termCond2Type <> "None" Then
            ...
            term2Str = term2Str & termToStr(currentStep.termCond2Type,
    ...
    Sub addToChart(checkToOutput As CureCheck,
                   mainSheet As Excel.Worksheet,
                   dataSheet As Excel.Worksheet,
                   cNum As Integer,
                   seriesName As String,
                   startDataSet As DataSet,
                   Optional axisGroup As Integer = 1,
                   Optional lineDashed As Boolean = False,
    ...
    Function arr2Str(Of T)(inArr() As T,
                           ...
                           Optional rndDecimals As Integer = 1,
                           Optional calcDuration As Boolean = False) As String()
    ...
    Sub outputDataToColumn(dataSheet As Excel.Worksheet,
                           ByRef cNum As Integer,
                           title As String,
                           outputDataSet() As String,
                           Optional startVal As Integer = 0,
                           Optional endVal As Integer = -1,
    ...
    Function termToStr(termCondType As String,
                       ...
                       currentStep As CureStep) As String
